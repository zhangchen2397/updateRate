...
function actionDayAnalyze(data) {
    ...
    pool.getConnection(function(err, connection) {
        ...
        connection.query(query, ['手机腾讯网', '要闻', '2015-12-13'], function(err, result) {
            if (!result.length) {
                var query = 'INSERT INTO day_analyze(date, updateCount, web, category) VALUES("2015-12-13", ?, ?, ?)';
...
module.exports = {
    ...
    analyze: function (req, res, next) {
        var query = 'select * from report where web="手机腾讯网" and category="要闻"';
        pool.getConnection(function(err, connection) {
